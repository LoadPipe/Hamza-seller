    ...
    */
    ...
    > => {
        ...
        for (const variant of productDetails.variants) {
            ...
            const convertedPrices = await productCsvService.getPricesForVariant(
                baseAmount,
    ...
    const convertRowDataToProductDetails = async (
        ...
        const hasVariant = productCsvService.csvRowHasVariant(
            rowData,
    ...
    const convertCsvDataToUpdateProductInput = async (
        ...
        const optionNames: CreateProductProductOption_[] | null =
            await extractOptionNames(productDetails.variants);
        // console.log('optionNames: ' + JSON.stringify(optionNames));

        // console.log('POSTCheck5.1.3.3');

        const variants: UpdateProductProductVariantDTO[] = (await mapVariants(
            productDetails
    ...
    const convertCreateCsvData = async (
         ...
         */
        const productRows = await productCsvService.filterCsvProductRows(
            data,
    ...
    const updateProducts = async (
        ...
        const products: Product[] = await productService.bulkUpdateProducts(
            store.id,
