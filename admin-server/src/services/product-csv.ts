...
import { CreateProductProductVariantInput } from '@medusajs/medusa/dist/types/product';
...
class ProductCsvService extends MedusaProductService {
    ...
    async validateCsvProductRow(
        ...
        if (!isCreate && row['product_id'] && row['product_id'].trim() !== '') {
            const product = await this.productService_.getProductById(
                row['product_id']
            );
        ...
        if (isCreate && row['category'] && row['category'].trim() !== '') {
            const categoryId = await this.productService_.validateCategory(
                row['category']
            );
            if (!categoryId) {
        ...
        if (isCreate && row['handle'] && row['handle'].trim() !== '') {
            const product = await this.productService_.getProductByHandle(
                row['handle']
            );
            if (product) {
                return (
	...
	*/
    ...
    public async convertRowDataToProductDetails(
        ...
        rowData.metadata = rowData.thumbnail
            ? {
                  imgUrl: rowData.thumbnail.startsWith('http')
                      ? rowData.thumbnail
        ...
        csvData.forEach((row) => {
            if (row.thumbnail) {
                row.metadata = {
                    imgUrl: row.thumbnail.startsWith('http')
                        ? row.thumbnail
                        : baseImageUrl + row.thumbnail,
                };
            }
        });
    ...
    public async convertCsvDataToCreateProductInput(
        ...
        const productDetails: ProductDetails =
            await this.convertRowDataToProductDetails(
                rowData,
                csvData,
                store,
                baseImageUrl
        ...
        const optionNames = await this.extractOptionNames(
    ...
    public async updateProducts(
        ...
        const products: Product[] =
            await this.productService_.bulkUpdateProducts(
                store.id,
