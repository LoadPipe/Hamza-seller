...
class NotificationDataService extends TransactionBaseService {
    ...
    async fetchData(event, data, attachmentGenerator) {
        ...
        switch (noun) {
            ...
            case 'gift_card':
                return await this.getGiftCardData(
                    event,
                    data,
                    attachmentGenerator
                );
            ...
            case 'restock-notification':
                return await this.getRestockNotificationData(
                    event,
                    data,
                    attachmentGenerator
                );
    ...
    async getOrderData(event, data, attachmentGenerator) {
        ...
        if (verb === 'refund_created') {
            ...
            const refund = order.refunds.find(
                (refund) => refund.id === data.refund_id
            );
            return {
                order,
                refund,
            ...
            if (
                ...
                const returnRequest = await this.returnService_.retrieve(
                    data.return_id,
                    {
                        relations: [
                            'items',
                ...
                let items = await Promise.all(
                    returnRequest.items.map(async (i) => {
                        const found: any = allItems.find(
                            (oi) => oi.id === i.item_id
                        );
                        found.quantity = i.quantity;
                        found.thumbnail = this.normalizeThumbUrl_(
                            found.thumbnail
                        );
                        found.totals =
                            await this.totalsService_.getLineItemTotals(
                                found,
                                order,
                                {
                                    include_tax: true,
                ...
                if (returnRequest.shipping_method) {
                    ...
                    shippingTotal =
                        base +
                        returnRequest.shipping_method.tax_lines.reduce(
                            (acc, next) => {
                                return Math.round(
                                    acc + base * (next.rate / 100)
                                );
                            },
                            0
                        );
                ...
                let items = await Promise.all(
                    order.items.map(async (i: any) => {
                        i.totals = await this.totalsService_.getLineItemTotals(
                            i,
                            order,
                            {
                                include_tax: true,
    ...
    async fetchAttachments(event, data, attachmentGenerator) {
        switch (event) {
            ...
            case 'order.return_requested': {
                ...
                if (shipping_method) {
                    const provider =
                        shipping_method.shipping_option.provider_id;
                ...
                if (
                    ...
                    const base64 =
                        await attachmentGenerator.createReturnInvoice(
                            data.order,
